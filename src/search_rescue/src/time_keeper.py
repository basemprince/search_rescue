...
class time_keeper:
    ...
    def tp_callback(self, data):
        ...
        # List 
        List=[self.robot_count,self.time_elapsed,self.counter0,self.counter1,self.counter2,self.percent_discovered]
        
        with open(r'time_keeper.csv', 'a', newline='') as csvfile:
            fieldnames = ['robot_count','time_taken','waypoint_count_0','waypoint_count_1','waypoint_count_2','perc_disc']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

            writer.writerow({'robot_count':List[0], 'time_taken':List[1],'waypoint_count_0':List[2],'waypoint_count_1':List[3],'waypoint_count_2':List[4],'perc_disc':List[5]})
